...
Simple explicit joins are supported by adding `join=EntityName:EntityAlias:JoinType:LeftJoinField:JoinCondition:RightJoinField`, for example for a join of individual contacts onto their organisations `join=Contact:contact:INNER:organization_name:=:contact.display_name`. All fields of the joined contacts will be available at `contact.*`
